...
It follows directly from Definition~\ref{def:gf-addition} that addition in $\C\llbracket X \rrbracket$ is:
\begin{itemize}
...
\end{thm}
\begin{proof}
Let $\frac{1}{f} \longleftrightarrow \{b_n\}$. From Definition~\ref{def:gf-multiplication} and Lemma~\ref{lem:gf-id-c}, we have
\begin{align*}
    &\begin{cases}
        a_0 b_0 &= 1\\
...
We have defined basic arithmetic operations in $\C\llbracket X\rrbracket$ and proven a few properties about them: we can add, subtract, multiply, and divide by invertible generating functions in the ways we are accustomed to. From an algebraic view, addition and multiplication in $\C\llbracket X\rrbracket$ forms an integral domain, and further, a unique factorisation domain \autocite[873]{niven}.
...
\textbf{(shifting)} Let $f \longleftrightarrow \{a_n\}, m \in \N$. Then $X^m \cdot f \longleftrightarrow \{\overbrace{0, 0, \cdots, 0}^{m\text{ zeros}}, a_0, a_1, a_2, \cdots\}$. For simplicity, we define $a_{n}$ to be $0$ for any negative integer $n$. Thus $X^m \cdot f \longleftrightarrow \{a_{n - c}\}$.
\begin{alignat*}{7}
...
By Theorem~\ref{thm:gf-mult-inv}, $(1 - \lambda X) \cdot \dfrac{1}{1 - \lambda X} = 1$. Also, by Remarks~\ref{rem:gf-scaling} and~\ref{rem:gf-shifting}, we can view this product as generating $\{a_n - \lambda a_{n - 1}\}$. Then, by Lemma~\ref{lem:gf-id-c},
\begin{align*}
    \iff &\begin{cases}
    a_0 - \lambda a_{-1} &= 1\\
...
We then find $(\frac{1}{f^m})'$ with the product rule, remembering from Remark~\ref{rem:gf-constant-deriv} that $1' = 0$:
\begin{align*}
...
\end{cor}
\begin{proof}
Let $f = \dfrac{1}{(1 - \lambda X)^m}$. By Lemma~\ref{lem:gf-power-rule}, $f' = \dfrac{m \lambda}{(1 - \lambda X)^{m + 1}}, f'' = \dfrac{m(m + 1) \lambda^2}{(1 - \lambda X)^{m + 2}}, \dots$, 
\[
f^{(n)} 
...
\begin{table}[h]
    ...
    & see Corollary~\ref{cor:gf-1-lambdaX-raised-to-m}\\
...
Recalling what Remarks~\ref{rem:gf-scaling} and~\ref{rem:gf-shifting} tell us about scaling and shifting, we can view the left hand side of the equation as generating the sequence $\{q_0 a_n + q_1 a_{n - 1} + \cdots + q_k a_{n - k}\}$. Then, Definition~\ref{def:gf-eq} allows us to identify each term and establish the following relations:
...
From Section~\ref{sec:gf-summary}, we know that $\dfrac{1}{(1 - \lambda X)^m}$ for $m \in \N$ and $\lambda \in \C^*$ generates $\{\binom{m + n - 1}{n} \lambda^n\}$. This shares many similarities with Theorem~\ref{thm:solve-lin-homo-recur}: the constants $\alpha_{i, j}$ are determined by the initial conditions; for $\lambda_i$ of multiplicity $m_i + 1$, $\alpha_{i, m_i + 1} \binom{m_i + n}{n} \lambda^n + \cdots + \alpha_{i, 1} \binom{n}{n} \lambda^n = R \cdot \lambda^n$, where $R = \alpha_{i, 1} + \cdots + \alpha_{i, m_i + 1}(m_i + n)(m_i + n - 1)\cdots(1 + n)/(m_i(m_i - 1)\cdots1)$ is a polynomial in $n$ of degree $m_i$, contributes to the solution.
