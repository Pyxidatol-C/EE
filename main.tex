...
\begin{thm}
Let $a_n = c_{k - 1}a_{n - 1} + c_{k - 2}a_{n - 2} + \cdots + c_0 a_{n - k} + g_n$ for $n \geq k$. The generating function of $\{a_n\}$ takes the form $\dfrac{P}{Q}$ (with $\mathrm{deg}(P) < \mathrm{deg}(Q)$) if and only if \[g_n = \sum_{j = 1}^{m} {\alpha_j}^n P_j\]
