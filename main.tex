...
The quotient rule $\left(\dfrac{f}{g}\right)' = \dfrac{f' g - f g'}{g^2}$ can be proved in the same manner. From $\frac{f}{g} \cdot g = f$,
\begin{align*}
    &\left(\frac{f}{g}\right)' + \frac{f}{g} \times g' = f'\\
...
\begin{thm}
Let $a_n = c_{k - 1}a_{n - 1} + c_{k - 2}a_{n - 2} + \cdots + c_0 a_{n - k} + g_n$ for $n \geq k$. The generating function of $\{a_n\}$ takes the form $\dfrac{P}{Q}$ if and only if \[g_n = \sum_{j = 1}^{m} {\alpha_j}^n P_j\]
